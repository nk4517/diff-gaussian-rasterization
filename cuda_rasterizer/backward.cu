	...
	float* __restrict__ dL_ddepths)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++) {
			...
			dL_dmean2D_shared[tid].x = skip ? 0.f : dL_dmean2D_x;
			dL_dmean2D_shared[tid].y = skip ? 0.f : dL_dmean2D_y;
            dL_dmean2D_shared[tid].z = skip ? 0.f : abs(dL_dmean2D_x);
            dL_dmean2D_shared[tid].w = skip ? 0.f : abs(dL_dmean2D_y);
